...
def check_timezone_configuration() -> dict[str, str]:
    ...
    if hasattr(settings, "CELERY_TIMEZONE") and settings.CELERY_TIMEZONE != settings.TIME_ZONE:  # type: ignore[misc]
        recommendations.append(
            f"CELERY_TIMEZONE ({settings.CELERY_TIMEZONE}) differs from "  # type: ignore[misc]
            f"TIME_ZONE ({settings.TIME_ZONE}). Consider aligning them."
        )

    if hasattr(settings, "CELERY_ENABLE_UTC") and not settings.CELERY_ENABLE_UTC and settings.TIME_ZONE != "UTC":  # type: ignore[misc]
